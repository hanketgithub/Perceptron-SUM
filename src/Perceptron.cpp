...
// Derivative of identity (for backprop)
...
float Neuron::forward(const vector<float> &input) {
  ...
  float output = identity(input_sum);
  return output;
}
...
void train(vector<vector<float>> &X, vector<float> &Y, Neuron &hidden1, Neuron &hidden2, Neuron &output_neuron, int epochs, const float learning_rate) {
  for (int epoch = 0; epoch < epochs; ++epoch) {
    ...
    for (size_t i = 0; i < X.size(); i++) {
      ...
      float d_output = 2 * error * identity_derivative(output_neuron.getInputSum());
      ...
      float d_hidden1 = d_output * w0 * identity_derivative(hidden1.getInputSum());
      float d_hidden2 = d_output * w1 * identity_derivative(hidden2.getInputSum());
